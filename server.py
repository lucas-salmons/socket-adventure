...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def room_description(self, room_number):
        ...
        return ["You are in the room with the white wallpaper.",
                "You are in the room with the green wallpaper.",
                "You are in the room with the brown wallpaper.",
                "You are in the room with the mauve wallpaper.", ][room_number]
    ...
    def route(self):
        ...
        :return: None
        """
        recieved = self.input_buffer.split('')
        # first work acts a command/prefix
        command = recieved.pop(0)
        arguments = ''.join(recieved)
        # create dispatch dict based on the self.method that curresponds
        # with command str, then evaluate with arguments
        {
            'quit': self.quit,
            'move': self.move,
            'say': self.say
